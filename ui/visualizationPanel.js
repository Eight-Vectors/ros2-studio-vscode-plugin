...
class VisualizationPanel {
  ...
  //           <div class="header">
  //             <div class="title">Topic: ${this._topicName} (${this._messageType})</div>
  //             <div class="view-mode-selector">
  //               <button class="view-mode-btn ${this._viewMode === 'graphical' ? 'active' : ''}"
  //                       onclick="changeViewMode('graphical')">Graphical</button>
  //               <button class="view-mode-btn ${this._viewMode === 'raw' ? 'active' : ''}"
  //                       onclick="changeViewMode('raw')">Raw Data</button>
  //               <button class="view-mode-btn ${this._viewMode === 'both' ? 'active' : ''}"
  ...
  //         </div>
  //         <div class="raw-data-panel ${this._viewMode !== 'both' ? 'hidden' : ''}" id="rawDataPanel">
  //           <div class="header">
  ...
  //               // Show raw data in main panel
  //               const mainRawData = document.getElementById('raw-data').cloneNode(true);
  //               mainRawData.id = 'main-raw-data';
  //               mainRawData.style.padding = '20px';
  //               canvasContainer.innerHTML = '';
  //               canvasContainer.appendChild(mainRawData);
  //               canvasContainer.classList.remove('hidden');
  //               break;
  //             case 'both':
  ...
  //             // Use collision geometry (more reliable than visual for basic shapes)
  ...
  //                   // URDF cylinders are Z-aligned, Three.js are Y-aligned
  //                   if (linkName.includes('wheel')) {
  //                     // For wheels, no rotation needed - they should be Y-aligned like Three.js cylinders
  //                     // This makes them horizontal and able to roll
  //                   } else {
  //                     // For other cylinders (like sensors), rotate around X-axis
  ...
  //           // Process joints to build hierarchy
  //           const joints = xmlDoc.querySelectorAll('joint');
  //           joints.forEach(joint => {
  ...
  //                 if (rpy) {
  //                   const rot = rpy.split(' ').map(parseFloat);
  //                   childLink.rotation.set(rot[0], rot[1], rot[2]);
  //                 }
  //               }
  //             }
  //           });
